...
import { mockWorkingShift, mockBaseUrl } from '../mock-data';
...
describe('WorkingShiftsService', () => {
  ...
  beforeEach(() => {
    mockHttp = { 
      get: jasmine.createSpy('get').and.returnValues(of([mockWorkingShift]), throwError('error'))
    }
    mockErrorService = {
    ...
    service = TestBed.get(WorkingShiftsService);
  ...
  it('#fetch should call http get with params', () => {
    const expectedParams = `${baseUrl}${apiPath.workingShifts}`;
    service.fetch();
  ...
  it('#fetched observable should emit expected values', () => {
    ...
    scheduler.run(({ expectObservable }) => {
      ...
      const expectedValue = { a: [mockWorkingShift] };
