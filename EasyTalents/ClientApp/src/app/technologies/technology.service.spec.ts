...
import { mockTechnology, mockBaseUrl } from '../mock-data';
...
describe('TechnologyService', () => {
  ...
  beforeEach(() => {
    mockHttp = { 
      get: jasmine.createSpy('get').and.returnValues(of([mockTechnology]), throwError('error'))
    }
    mockErrorService = {
    ...
    service = TestBed.get(TechnologyService);
  ...
  it('#fetch should call http get with params', () => {
    const expectedParams = `${baseUrl}${apiPath.technologies}`;
    service.fetch();
  ...
  it('#fetched observable should emit expected values', () => {
    ...
    scheduler.run(({ expectObservable }) => {
      ...
      const expectedValue = { a: [mockTechnology] };
